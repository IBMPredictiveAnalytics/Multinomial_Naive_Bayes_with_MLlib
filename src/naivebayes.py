script_details = ("naivebayes.py",0.5)
...
lp = dmt.extractLabelledPoint(df,target,predictors).map(lambda x:x[1])

# build the decision tree model
from pyspark.mllib.classification import NaiveBayes

model = NaiveBayes.train(lp,lambda_param)

build_report = mbr.report(lp.count(),lp.getNumPartitions(),
    predictors,datamodel,target,"classification",
    settings=[("Algorithm","Naive Bayes",[("lambda",lambda_param)])])

print(build_report)

model.save(sc, modelpath)

model_metadata = { "target":target, "predictors":predictors, "datamodel": datamodel }
